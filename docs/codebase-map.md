...
| `MemberItem.vue` | 成员条目组件：头像、昵称、属性标签、我的高亮、警告图标、编辑入口、踢出按钮，供详情页三种成员视图复用 |
...
| `ActivityDetailView.vue` | 活动详情页：操作按钮（加入活动/开始分组 + 分享链接 + 更多▼）+ 分组规则展示 + 成员列表（分组/尚未分组/平铺三视图，复用 MemberItem）+ 踢出成员（管理成员开关）+ 属性选择弹框 + 确认对话框；重新分组/解除分组/编辑/查看日志/删除/编辑我的信息在更多菜单中 |
